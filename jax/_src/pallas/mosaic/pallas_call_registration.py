...
def pallas_call_gpu_lowering_rule(
    ctx: mlir.LoweringRuleContext, *in_nodes,
    jaxpr: jax_core.Jaxpr,
    name: str,
    which_linear: tuple[bool, ...],
    ...
    mosaic_params: dict[str, Any] | None = None,
    **compiler_params: Any):
  """Lowers a pallas_call to a Mosaic GPU custom call."""
  if interpret:
    return mlir.lower_fun(pallas_call_p.impl, multiple_results=True)(
        ctx, *in_nodes, jaxpr=jaxpr, name=name, out_shapes=out_shapes,
        in_shapes=in_shapes,
        ...
        grid_mapping=grid_mapping, **compiler_params)
  if input_output_aliases:
    raise NotImplementedError("`input_output_aliases` not supported.")
  if debug:
    print(jaxpr)
  mesh = None  # TODO: Raise a better error for collectives on GPU
  with ir.Context() as mlir_ctx, ir.Location.unknown(mlir_ctx):
    tpu.register_dialect(mlir_ctx)
    if mosaic_params is None:
      mosaic_params = {}
    ...
    if debug:
      print(mosaic_module)
  raise NotImplementedError(mosaic_module)

mlir.register_lowering(pallas_call_p, pallas_call_gpu_lowering_rule,
                       platform="gpu")
